...
export function getWeatherIcon(code: number, size: number, isDay = true): JSX.Element {
  switch (code) {
    case 1000: // Sunny
      return <div className="relative w-12 h-12 m-auto">{!isDay ? <MoonIcon className="w-12 h-12 text-yellow-400" fontSize={size} /> : <SunIcon className="w-12 h-12 text-yellow-400 animate-spin-slow" fontSize={size} />}</div>;

    case 1003: // Partly cloudy
      return (
        <div className="relative w-12 h-12 m-auto">
          {!isDay ? <MoonIcon className="absolute w-8 h-8 text-yellow-400 -rotate-90 -left-1 z-0" fontSize={size} /> : <SunIcon className="absolute w-8 h-8 text-yellow-400 -top-1 -left-1 z-0 animate-spin-slow" fontSize={size} />}
    ...
    case 1009: // Cloudy / Overcast
      return (
        <div className="relative w-12 h-12 m-auto">
          <CloudIcon className="w-12 h-12 text-blue-400" fontSize={size} />
        </div>
      );
